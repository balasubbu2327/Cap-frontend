...
const Product = ({
  ...
  const deleteRequest = async () => {
    const res = await axios
      .delete(`https://inventory-dgy3.onrender.com/api/product/${id}`)
    ...
    <Box>
      ...
      >
        <CardHeader
          ...
          subheader={date}
        />
        <CardMedia component="img" height="180" image={image} alt="model" />
        <br />
        <hr />
        <CardContent>
          ...
          {isUser && (
            <Box display="flex">
              <IconButton onClick={handleEdit} sx={{ marginRight: "auto" }}>
                <ModeEditOutlineIcon color="primary" fontSize="large" />
              </IconButton>
